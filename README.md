...
TODO релоад списка книг
TODO генерация списка книг - если нет в data - генерить и сохранять. иначе брать оттуда.
так же правильно выводить что делаю (Calculate CRC32)


//TODO сейчас сломана программа из-за детектора уникальности файлов.
--Решение:
--0. При работе проверять файлы на уникальность с учётом хвостов (см. пункт 1) и валить ошибку если что.
--1. BookRecord так же хранит другие поля. Например BookID. Например, то, что добавляется к названию файла (Rus), (v1.0), (Original), (Old), итд
--2. Экспортировать как CSV
--3. Читать из CSV
--4. Считать CRC32 только для тех файлов, для которых его нет
--5. CRC32 сейчас не использовать. возможно на будущее для поиска дубликатов.
--6. Считать, что файлы уникальны по длине в пределах одной книги (мало вероятен баг с DOC, сильно вероятен при ROM'ах.
--...
--слишком трудоёмко. поскольку всё будет лежать на внешке - нет смысла что-то исключать.
[v] выключить (закомментить) всё что касается списков. для кнопки - алерт о том, что не работает.
...
[v] у DATA ещё один филд - tail. при поиске уникального имени файла прибавлять его.
...
= audit - уникальность имён файлов
= audit - не проверять издателей у гайдов
