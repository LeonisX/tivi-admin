...
- Обновить func.php - добавить все платформы. Так же пересчитать рубрики
...
= Для мануалов, гайдов и документов: краткий текст - основной, а то что за HR - прятать в спойлер - если есть
- Всё ещё думать про журналы. Может ещё один блок для описания серии???
= возможная проблема при делениии текста на сокращённый и полный - надо чистить DIV при импорте


= TODO склеивать краткое и полное описание, <hr>, закидывать в базу Calibre
= TODO обеспечить поддержку новой структуры

= Упоминания atari jaguar в журналах - lower case
...
- Пересчитывать категории медиа вручную

= генерация изменений для всех платформ
...
= для мануалов, комиксов, журналов - устанавливать картинку реального мануала

= http://tv-games2018.ru/media/open/consoles_search.html - сортировка по названию
= http://tv-games2018.ru/media/open/gb_citation.html - сортировка, возможно
= http://tv-games2018.ru/media/open/megadrive_guides.html - сортировка

- баг домашняя мастерская радиолюбителя - битый файл, берёт 7z вместо ISO.




- Из не решённого - как отбирать файлы для сайта? Брать первый? Приоритеты размер, DJVU? Другие решения? Таблица где принудительно отбираю?
идея такая:
1. Генерить табличку:

# Sega Manuals
+ sega.pdf [size]// + - automatic if one file
+ sega.djvu (in sega.rar) [size]
...
Если исключаю - ставлю минус. Если несколько плюсов - предоставить возможность скачать остальные файлы тоже.

Эта таблица стоится на базе работы всех архиваторов - нужен метод, который не распаковывает а просто берёт имена всех файлов

Нужен флажок - распаковывать все или только для сайта

Далее, когда работают архиваторы - если только для сайта - проверяют архивы, и если в табличке плюс - распаковывает и пишет распакованное имя.

После работы архиваторов - сохранять табличку

При работе использовать эту табличку в качестве основы.

Будут вопросы - что делать с ссылками?

//TODO load books list on start
//TODO use this list in books generation
