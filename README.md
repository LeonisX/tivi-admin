...
выключить (закомментить) всё что касается списков. для кнопки - алерт о том, что не работает.

в коде, если книга, журнал или комикс - хардкодить ссылку на внешний источник.
вероятно кастомизировать кнопку скачивания, если ссылка

для остальных документов предусмотреть вывод на скачку остальных файлов.
порядок - оригинальный (DATA ID, а в ррхивах по порядку следования).
